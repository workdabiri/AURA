...
- **Phase 2 (Public Website) is IN PROGRESS — AURA-201 (public layout + i18n shell + settings selector) MERGED at `f17b429` and AURA-202 (public properties listing + `GET /api/properties` + featured) MERGED at `1d4c514`** (2 of 7 Phase 2 tasks done). The public site shell, settings-driven footer, properties listing page, public properties API, PropertyCard, and homepage featured section now exist. The next Phase 2 task is **AURA-203 (Property detail + stakeholder visibility)** — **not started; read-only discovery only**, requires a new session + per-task discovery/planning approval (AURA-203 likely requires Opus 4.8 review — public property-detail data boundary + stakeholder visibility). AURA-204–207 (areas, legal, SEO/noindex, about) remain not started. **The following do NOT exist yet:** property **detail** route/page (`/[locale]/properties/[slug]` + `GET /api/properties/[slug]`), public **stakeholder projection**, **contact routing**, **lead/WhatsApp routes**, **admin** property management, **media upload**, **areas overview**, **legal pages**, **SEO/noindex/Lighthouse**, and **cinematic/GSAP** work — only the shared layout shell, homepage (+ featured section), and the properties listing surface exist
