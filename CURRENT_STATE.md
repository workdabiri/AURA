...
- **Phase 2 (Public Website) is IN PROGRESS — AURA-201 (`f17b429`), AURA-202 (`1d4c514`), and AURA-203 (`b2f6129`) MERGED** (3 of 7 Phase 2 tasks done). The public site shell, settings-driven footer, properties listing page + API, PropertyCard, homepage featured section, and the **public property detail surface** (`/[locale]/properties/[slug]` + `GET /api/properties/[slug]`, safe public stakeholder projection, contact routing, off-plan block, media gallery) now exist. The next Phase 2 task is **AURA-204 (Areas overview — DAL + `GET /api/areas`)** — **not started; read-only discovery only**, requires a new session + per-task discovery/planning approval. AURA-205–207 (legal, SEO/noindex, about) remain not started. **The following do NOT exist yet:** **areas overview** not started (`/[locale]/areas`), **`GET /api/areas`** not started (both AURA-204), **legal pages** (AURA-205), **SEO/noindex/Lighthouse** (AURA-206), **about page** (AURA-207), **admin property management** (Phase 3), **media upload** (AURA-304), **lead form** (AURA-401), **WhatsApp click tracking** (AURA-405), **similar properties**, and **cinematic/GSAP** work (AURA-502)
