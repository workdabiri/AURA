...
/**
 * AURA-307 — shared helpers for the admin legal Route Handlers.
 *
 * NOT a route module (underscore-prefixed → never routed). `server-only`-tainted via the auth
 * barrel. Mirrors the AURA-305/306 admin helpers so each `/api/admin/legal*` handler:
 ...
 *
 * Both MVP admin roles (`super_admin` and `client_admin`) may manage legal pages (owner decision),
 * so this uses the any-admin guard — NEVER the super-admin-only guard.
 */
