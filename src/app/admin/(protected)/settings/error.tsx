...
/**
 * Admin settings error state (`/admin/settings`) — AURA-306 (D-44 error + retry state).
 *
 * Client error boundary for the settings segment. Shows a generic message (no internal detail) and
 * a retry button that re-renders the segment. No Supabase/DAL/services import.
 */
  ...
  return (
    <div className="px-6 py-8 md:px-10" role="alert">
      ...
      <p className="mt-2 max-w-xl text-small text-text-secondary">
        We couldn’t load the settings. This may be a temporary issue.
      </p>
      <button
        type="button"
        onClick={reset}
