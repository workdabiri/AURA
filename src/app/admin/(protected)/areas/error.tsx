...
/**
 * Admin areas error state (`/admin/areas`) — AURA-305 (D-44 error + retry state).
 *
 * Client error boundary for the areas segment. Shows a generic message (no internal detail) and a
 * retry button that re-renders the segment. No Supabase/DAL/services import.
 */
export default function AdminAreasError({ reset }: { error: Error; reset: () => void }) {
  return (
    <div className="px-6 py-8 md:px-10" role="alert">
      ...
      <p className="mt-2 max-w-xl text-small text-text-secondary">
        We couldn’t load the areas list. This may be a temporary issue.
      </p>
      <button
        type="button"
        onClick={reset}
