...
| `db-tests` | `.github/workflows/ci.yml` — `jobs.db-tests` (added in **AURA-107**) | **Should be Yes** — added in AURA-107; live DAL/security/integration tests against the Dockerized Supabase stack. **Green on PR #23 but not yet added to the `develop` rule — owner action required (see below).** |
...
- [ ] **Require status checks to pass before merging**
  ...
  - [x] Currently required: **`quality`**, **`e2e`**, **`analyze (javascript-typescript)`**, **`CodeQL`** (verified via GitHub API on 2026-06-20)
  - [ ] **Add `db-tests`** (AURA-107) to the required checks — **outstanding owner action**
