...
> **Status:** Implemented in **AURA-307** (merged `74da365`, PR #51) — the Phase 3 exit gate. All five admin legal routes are admin-only — each calls `requireAdmin()` directly via `withAdmin` (both `super_admin` and `client_admin`; the `(protected)` layout guards pages, not Route Handlers). Admin legal CRUD uses a **request-scoped authenticated admin Supabase client + RLS — no service role** (`src/dal/legal.dal.ts`); the only service-role path remains the AURA-303 append-only audit writer (`legal_page_created` / `legal_page_published` / `legal_page_archived`; entity type `legal_page`; metadata carries safe identifiers only — slug / status transition / version — never the legal title or body). Slugs are restricted to **`privacy` / `terms`** (no arbitrary slugs). Content is **Markdown only**; raw / unsafe HTML is **rejected at write time** (Zod refine + a DAL backstop) as well as at render — the existing `SafeMarkdown` (`react-markdown` + `rehype-sanitize`) remains the only renderer (no `dangerouslySetInnerHTML`, no `rehype-raw`, no `marked`, no DOMPurify/raw-innerHTML pipeline). Ordinary prose containing `data:` / `personal data:` / `metadata:` is accepted; dangerous protocol link targets such as `[bad](javascript:alert(1))` and `<javascript:alert(1)>` are rejected (blocker fix `0b07090`, Opus re-review APPROVE). **Row-per-version lifecycle** (`draft` → `published` → `archived`): only drafts are editable; publishing **archives the currently published row for the same slug, then promotes the selected draft** with `version = max(existing version for slug) + 1` and `published_at = now`. Draft PATCH is **not** audited. The publish/audit sequence is **non-atomic (owner-accepted; fails loud, tested)** — no migration, no DB RPC; existing schema + RLS only. **Archive only — no hard delete.**
