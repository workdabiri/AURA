...
admin **settings** routes/page (AURA-306, merged `86e8b36`), and admin **legal** routes/page (AURA-307,
merged `74da365`) are now **implemented**; the remaining admin route above (**leads**) is **not yet
implemented** — its dashboard nav link 404s until built; **no placeholder route file was created** for it.

---

## Feature Spec: Property Management

> **Status (AURA-303, merged `a6cb178`):** admin property **CRUD is implemented** — create / edit / publish / archive / duplicate via guarded `/admin/properties` pages and the role-guarded `/api/admin/properties*` routes (both `super_admin` and `client_admin`). The **publish checklist** (cover image + alt text + required fields + taxonomy + off-plan + price-visibility + bedrooms-by-type) and the **duplicate** and **archive** flows are implemented and audit-logged. **Media upload was deferred to AURA-304 (now implemented — see below)**; in AURA-303 the publish checklist only read existing `property_media` rows. **Stakeholder management is not implemented.** No hard delete and no `published → draft` unpublish (archive is the MVP way to remove a listing from public view, D-32).

> **Status (AURA-304, merged `631bd29`):** admin **property media upload / update / delete is implemented**. The **admin media manager** (`src/components/admin/PropertyMediaManager.tsx`, wired into the property edit page) lets admins upload images and floorplan images, edit **alt text**, and select the **cover** image. **Media validation is implemented**: single-file upload only; allowed MIME `image/jpeg` / `image/png` / `image/webp`; 10MB max; server-built UUID-based storage path (MIME-derived extension; original filename never trusted). The **single-cover rule** (app-level), floorplan-cannot-be-cover, and image-only-cover are enforced; deleting the cover does not auto-pick another (publish stays blocked until another cover image with alt text exists). Storage writes use a request-scoped authenticated admin Supabase client + existing RLS (no service role). **Out of scope / deferred:** signed URLs, CDN revocation, video / 360 / virtual-tour, image processing / resizing / transcoding, manual reordering, multi-file upload / drag-drop, and media audit actions.
...
**Media:** Images and floorplan images only; UUID-based storage path; 10MB max; type validation; alt text required before publish. **(Implemented — AURA-304, merged `631bd29`: admin upload/update/delete + media manager UI; signed URLs / video / 360 / image processing / reordering / multi-upload remain out of scope.)**
...
**Public read (AURA-205, merged `3d6a7e0`):** the public legal pages are live at `/en/privacy` and `/en/terms`, backed by `GET /api/legal/[slug]`. Supported slugs: `privacy`, `terms`. Reads are **published-only** (draft/archived/missing → `404` publicly) through an anon-client DAL behind the RLS public-read boundary (the DAL re-asserts published; explicit public-safe column allowlist). The public DTO is `{ slug, title, content, effectiveDate }` only — `content` is raw Markdown in the DTO and is rendered safely **server-side via `react-markdown` + `rehype-sanitize`** (no `rehype-raw`, no `dangerouslySetInnerHTML`, no unsafe raw HTML path), honoring D-12. **No admin legal editing and no SEO/noindex in AURA-205** — admin draft→publish→archive editing is AURA-307 (now merged, see below); SEO/noindex is AURA-206.

> **Status (AURA-307, merged `74da365`, PR #51) — Phase 3 exit gate:** admin legal page management is **implemented**. Delivered: a guarded admin legal UI under `src/app/admin/(protected)/legal/**` (list + `new` + `[id]/edit`, with D-44 loading/error/validation/success states; content editor is a **Markdown textarea only** — no rich-text / raw-HTML editor — whose optional preview reuses the public `SafeMarkdown`), the admin-only routes `GET/POST /api/admin/legal` + `PATCH /api/admin/legal/[id]` + `POST /api/admin/legal/[id]/publish` + `POST /api/admin/legal/[id]/archive` (each calls `requireAdmin()` directly via `withAdmin`; both `super_admin` and `client_admin`; all Zod-validated), the pure domain contract `src/domain/legal/admin.ts` (slug allowlist `privacy` / `terms`; the **write-time D-12 content guard**; the version-bump rule), the admin DAL in `src/dal/legal.dal.ts` (request-scoped authenticated admin client + RLS — **no service role**; the public `getPublishedLegalPage` selector is intact), and the `legal_page_created` / `legal_page_published` / `legal_page_archived` audit actions in `src/dal/audit-logs.dal.ts` (metadata = safe identifiers only — slug / status transition / version — never the legal title or body). **Markdown only; no raw HTML** — unsafe content (`<script>` / `<iframe>` / raw tags / event handlers / dangerous protocol link targets) is **rejected at write time** and at render; the existing `SafeMarkdown` remains the only renderer (no `dangerouslySetInnerHTML` / `rehype-raw` / `marked` / DOMPurify). **Row-per-version lifecycle** (`draft` → `published` → `archived`): only drafts are editable; publishing archives the currently published row for the same slug then promotes the selected draft (`version = max(existing version for slug) + 1`, `published_at = now`); draft PATCH is **not** audited; the publish/audit sequence is non-atomic (owner-accepted; fails loud, tested). **Archive only — no hard delete. Public legal reads remain published-only; draft/archived pages stay hidden publicly.** **No migration, no DB RPC, no Supabase config change, no package/CI/branch-protection change** (existing schema + RLS only). Opus review: initial **REQUEST_CHANGES** (a `data:`-prose false-positive in the write-time guard), resolved by blocker fix `0b07090`; **re-review APPROVE, merge recommendation YES, no blockers**.
