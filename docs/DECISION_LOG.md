...
These were implicit in the pack and are now explicitly ratified.

| ID | Assumption | Ratified Value |
|---|---|---|
| A-01 | CI/CD platform | GitHub Actions |
| A-02 | Test database | Supabase CLI local stack for dev; Docker in CI. Do not mock the DB layer for DAL/integration tests. |
| A-03 | Frontend testing | Vitest (unit/integration/DAL/security) + Playwright (E2E/smoke) |
| A-04 | Email service | Resend |
| A-05 | Currency display | AED; no FX conversion in MVP |
| A-06 | Property slug | Derived from `title->>'en'`; collision-suffixed; **immutable after publish** |
| A-07 | Pagination server cap | **50 results per page maximum** |
| A-08 | Max image file size | 10MB per image |
| A-09 | Supported image formats | `image/jpeg`, `image/png`, `image/webp` |
| A-10 | Rate limit table TTL | 24 hours; scheduled cleanup via Supabase pg_cron or equivalent |
| A-11 | Price display currency | **AED-only in MVP; no FX conversion** |
