...
| 2026-06-29 | D-41 | Implemented (AURA-304, merged `631bd29`): admin property media upload/update/delete for **images + floorplan images only** (no video/360/virtual-tour). Media storage writes use a **request-scoped authenticated admin Supabase client + existing RLS — not the service role**; the single service-role path remains the AURA-303 audit writer. **Signed URLs / CDN revocation remain deferred** (public-read bucket gap documented in `DATA_MODEL.md` / `SECURITY_BASELINE.md`). The **single-cover rule is enforced at the app level in AURA-304**; a DB-level guarantee (partial unique index on `(property_id) where is_cover`) is possible **future hardening**, not current implementation. | Opus 4.8 review (PR #45) |
| 2026-06-29 | D-22 | Implemented (AURA-305, merged `aee1fda`): admin areas management — **add / edit / deactivate / reactivate only, no hard delete**. Area CRUD and the one representative area/community image upload run under a **request-scoped authenticated admin Supabase client + existing RLS — not the service role**; the single service-role path remains the AURA-303 audit writer. The area image **reuses the existing `property-media` bucket** under `areas/{area_id}/{image_id}.{ext}` (server-built, UUID-only path; MIME allowlist jpeg/png/webp; 10MB max; `upsert: false`), with the public URL stored in the existing `areas.image_url` — **no migration, no Supabase config change, no area media table, no gallery / multi-upload / drag-drop / image processing**. Slug is editable **only at create** (PATCH cannot change it). Admin-only property counts (`totalProperties` / `publishedProperties`) are computed from `properties.area_id` + `publish_status`, not stored. Public `/api/areas` stays **active-only**; inactive areas remain hidden; no public property counts / area-detail / public-areas redesign. Audit actions `area_created` / `area_updated` (deactivate/reactivate logged as `area_updated` with metadata; minimal audit-action-union extension; post-mutation/non-atomic, matching the AURA-303 pattern). **Known non-blocking follow-up:** the public-read bucket means a known UUID image URL stays fetchable after deactivation and old-image cleanup is best-effort (only the public URL is stored) — deferred to future signed-URL / object-revocation / storage-GC work. | Opus 4.8 review (PR #47) |
| 2026-06-30 | D-20, D-21, D-46, A-09 | Implemented (AURA-306, merged `86e8b36`, PR #49): admin settings management — admin edits **operational content only, never template/design architecture** (D-20/D-21). Editing is via a **server-side key-value allowlist of exactly the seven existing public footer keys** (`agency_name`, `agency_phone`, `agency_email`, `agency_whatsapp`, `agency_address`, `footer_tagline`, `social_links`) with **per-key Zod** (A-09); unknown / deferred keys and empty patches are rejected. `PATCH` is a **partial batch**; updates are **immediate + validated + audit-logged** (D-46) via `settings_updated` (entity type `settings`; metadata = changed key **names** only, never values). Admin GET/PATCH run under a **request-scoped authenticated admin Supabase client + RLS — not the service role**; the existing public safe selector keeps the service role and the single other service-role path remains the AURA-303 audit writer. The public footer reflects an updated value on the next request (no cache/revalidation system). **No settings delete path, no public settings API, no draft/publish flow, no migration, no Supabase config change.** Deferred keys/features (`logo_url`, `seo_*`, license/trust fields, `footer_content`/`footer_links`/`contact_us_content`, logo upload, SEO public-metadata wiring, design-token/theme/layout/motion editing) are out of AURA-306 scope. | Opus 4.8 review (PR #49) |
| 2026-07-01 | D-10, D-12 | Implemented (AURA-307, merged `74da365`, PR #51 — **Phase 3 exit gate**): admin legal page management — **draft → published → archived, row-per-version, audited**. Slugs limited to `privacy` / `terms` (no arbitrary slugs). Content is **Markdown only**; raw/unsafe HTML is **rejected at write time** (a pure domain guard on the Zod refine **and** re-asserted in the DAL before every insert/update) as well as at render — the existing `SafeMarkdown` (`react-markdown` + `rehype-sanitize`) remains the only renderer (D-12; no `dangerouslySetInnerHTML` / `rehype-raw` / `marked` / DOMPurify; the admin editor is a Markdown textarea). Publishing **archives the currently published row for the same slug, then promotes the selected draft** with `version = max(existing version for slug) + 1` and `published_at = now`; only drafts are editable; **archive only, no hard delete**. Admin legal CRUD runs under a **request-scoped authenticated admin Supabase client + RLS — not the service role** (service-role stays isolated to the audit writer). Audit actions added: exactly `legal_page_created` / `legal_page_published` / `legal_page_archived` (entity type `legal_page`; metadata = slug / status transition / version only, never the legal title or body); `legal_page_updated` **not** added (draft PATCH is not audited). The publish + audit sequence is **non-atomic (owner-accepted; fails loud, tested)**. **No migration, no DB RPC, no Supabase config change, no package/CI/branch-protection change** (existing schema + RLS only). Public legal reads stay published-only; draft/archived pages remain hidden publicly. Opus review: initial **REQUEST_CHANGES** (a `data:`-prose false-positive in the write-time guard, fixed in `0b07090` — ordinary `data:` / `personal data:` / `metadata:` prose accepted while dangerous protocol link targets stay rejected); **re-review APPROVE, merge recommendation YES, no blockers**. | Opus 4.8 review (PR #51) |
