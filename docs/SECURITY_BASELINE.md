...
**Implemented (AURA-307, merged `74da365`) — admin legal write boundary (Phase 3 exit gate):** admin legal authoring (`GET/POST /api/admin/legal`, `PATCH /api/admin/legal/[id]`, `POST /api/admin/legal/[id]/publish`, `POST /api/admin/legal/[id]/archive`, and the guarded UI under `src/app/admin/(protected)/legal/**`) satisfies D-12 by **rejecting unsafe/raw HTML at write time** (a pure domain guard used by the Zod refine **and** re-asserted in the DAL before every insert/update) in addition to the render-layer safety — the content editor is a **Markdown textarea only** and the existing `SafeMarkdown` (`react-markdown` + `rehype-sanitize`) is the only renderer (no `dangerouslySetInnerHTML` / `rehype-raw` / `marked` / DOMPurify). Ordinary prose containing `data:` / `personal data:` / `metadata:` is accepted, while dangerous protocol link targets (`[bad](javascript:alert(1))`, `<javascript:alert(1)>`) and raw tags / event handlers are rejected (blocker fix `0b07090`; Opus re-review APPROVE). Every admin legal handler runs `requireAdmin()` (both `super_admin` and `client_admin`); admin legal CRUD uses a **request-scoped authenticated admin client + RLS — no service role** (service-role stays isolated to the audit writer). Slugs are limited to `privacy` / `terms`; **archive only, no hard delete**; public legal reads remain published-only. **D-12 remains a merge blocker.**
