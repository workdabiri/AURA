...
**Status (AURA-306, merged `86e8b36`):** the admin **settings** test suite now exists across all layers — unit (`settings-admin-schema`: the exact seven-key editable allowlist + equality to the public allowlist, unknown/deferred-key rejection, empty-patch rejection, per-key Zod (non-empty trimmed strings, valid email, known social-platform URLs, social `{}` clears links, unknown platform stripping), and the `toSettingRows` allowlist backstop), integration (`admin-settings-api`: 401 unauth / 403 no-role with DAL+audit never called, 400 invalid JSON / unknown key / deferred key / invalid email / invalid social URL / empty patch, single-key and partial-batch writes, and the `settings_updated` audit emitting changed key **names only** — never values), security boundary (`admin-settings-boundary`: every route guarded via `withAdmin` → `requireAdmin` and **not** `requireSuperAdmin`; the route + helper never import the service-role client; the settings DAL admin region uses the request-scoped admin client while the public selector legitimately keeps the service role; no `.delete()` / no DELETE handler; the audit writer stays server-only + service-role and gains exactly `settings_updated`; no unguarded `src/app/admin/settings`; the `SettingsForm` imports no service-role / Supabase / DAL / services / storage), **live-DB DAL/RLS** (`admin-settings.dal`, gated `SUPABASE_LOCAL_TESTS=1`: an admin can SELECT/INSERT/UPDATE (upsert) settings and an upserted value reads back unchanged, while **anon can do none** of those and a hard DELETE is denied for admin), and e2e (`admin-settings`: unauthenticated `/admin/settings` → `/admin/login` runs unconditionally; the seeded-admin authenticated happy path — edit footer tagline → save → reflected on the public footer — gated behind `ADMIN_E2E_EMAIL`/`ADMIN_E2E_PASSWORD` and skipped in CI). The gated live suites ran in CI's `db-tests` job; CI checks green before merge (`CodeQL`, `analyze (javascript-typescript)`, `db-tests`, `e2e`, `quality`; Lighthouse advisory non-blocking).

**Status (AURA-307, merged `74da365`) — Phase 3 exit gate:** the admin **legal** test suite now exists across all layers — unit (`legal-admin-schema`: the `privacy` / `terms` slug allowlist, the D-12 write-time content guard incl. **accept** cases for ordinary prose like `5 < 10`, Markdown links, and `data:` / `personal data:` / `metadata:` and **reject** cases for `<script>` / `<iframe>` / raw tags / event handlers / HTML comments / dangerous protocol link targets (`[bad](javascript:alert(1))`, `<javascript:alert(1)>`), the create/update Zod schemas, and the `nextPublishedVersion` bump), integration (`admin-legal-api`: 401 unauth / 403 no-role with DAL+audit never called, the D-12 unsafe-HTML rejection incl. a realistic `data:` privacy-prose **accept** reaching `createLegalDraft`, the slug allowlist, DAL-result→HTTP mapping 201/200/404/409, and the create/publish/archive audits with draft PATCH **not** audited), security boundary (`admin-legal-boundary`: every route guarded via `withAdmin` → `requireAdmin` and **not** `requireSuperAdmin`; no service-role import in the legal routes/DAL; the DAL never `select('*')` and never `.delete()`; the audit union adds exactly the three legal actions and **not** `legal_page_updated`; no unguarded `src/app/admin/legal`; the legal components import no service-role / Supabase / DAL / services / storage), a public-boundary regression (`legal-public-boundary`: anon reads published-only; the D-12 renderer unchanged), **live-DB DAL/RLS** (`admin-legal.dal`, gated `SUPABASE_LOCAL_TESTS=1`: admin reads all statuses while anon reads published-only, admin can insert/update a draft, the partial unique index forbids two published rows per slug, the publish sequence preserves the published-only boundary, and hard DELETE is denied), and e2e (`admin-legal`: unauthenticated `/admin/legal*` → `/admin/login` unconditional; the seeded-admin authenticated create→publish→archive flow gated behind `ADMIN_E2E_EMAIL`/`ADMIN_E2E_PASSWORD` and skipped in CI). The gated live suites ran in CI's `db-tests` job; CI checks green before merge (`CodeQL`, `analyze (javascript-typescript)`, `db-tests`, `e2e`, `quality`; Lighthouse advisory non-blocking).
