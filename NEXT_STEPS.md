...
**Branch protection (unchanged by AURA-205):** `db-tests` remains required on `develop` — `develop` required checks are: `quality`, `e2e`, `analyze (javascript-typescript)`, `CodeQL`, `db-tests`. AURA-205 did not change branch protection.

**Immediate next action — AURA-206 read-only discovery only.** The next Phase 2 task is **AURA-206 (SEO basics + AUTEX noindex (D-42) + enable Lighthouse advisory CI)**: SEO metadata + the AUTEX `noindex` default + enabling the non-blocking Lighthouse advisory CI, reusing the AURA-201 layout shell and the AURA-202/203/204/205 patterns. It is **not started**. **Do not start AURA-206 implementation directly. Do not create the AURA-206 branch until discovery is complete and the owner approves.** Read-only discovery only; a new session + explicit per-task discovery/planning approval is required before any work begins. **AURA-206 owns SEO/noindex/Lighthouse** (D-42 AUTEX noindex; D-27 Lighthouse advisory).
...
### Phase 2 — Public Website — In progress (5/7)
...
| **AURA-206** | SEO basics + AUTEX noindex (D-42) + enable Lighthouse advisory CI | Not started — next; read-only discovery only; requires a new session + per-task discovery/planning approval |
| AURA-207 | About page + Phase 2 completion | Not started |
...
- Do not start AURA-206 implementation directly — AURA-206 (SEO basics + AUTEX noindex (D-42) + enable Lighthouse advisory CI; next Phase 2 task) is **read-only discovery only**; it requires a new session + explicit per-task discovery/planning approval before implementation
- Do not create the AURA-206 branch until discovery is complete and the owner approves
- Do not modify `develop` branch protection from a code/docs session — branch-protection changes are manual owner actions in GitHub Settings (unchanged by AURA-205; the `db-tests` required check remains in place)
- Do not create `.env` / `.env.local` files
- Do not create Stage 2 skills
- Do not auto-merge to `main`
- Do not implement further public pages (SEO/about) without per-task approval (AURA-206+)
