...
**Immediate next action — AURA-304 read-only discovery only.** With AURA-303 merged, the next task is **AURA-304 (Media upload — validation, UUID paths)** — the fourth Phase 3 task: admin media upload for images + floorplan images with type/size validation, UUID-based paths, and cover-image/alt-text enforcement (the first real consumer of the AURA-105 storage policies + media path contract). It is **not started**. **Do not start AURA-304 implementation directly. Do not create the AURA-304 branch until discovery is complete and the owner approves.** Read-only discovery only; a new session + explicit per-task discovery/planning approval is required before any work begins.
...
### Phase 3 — Admin Vertical Slice — In progress (3/7)
...
| **AURA-304** | Media upload — validation, UUID paths | Not started — **next**; read-only discovery only; requires a new session + per-task discovery/planning approval |
...
- Do not start AURA-304 implementation directly — AURA-304 (Media upload — validation, UUID paths; the fourth Phase 3 task) is **read-only discovery only**; it requires a new session + explicit per-task discovery/planning approval before implementation
- Do not create the AURA-304 branch until discovery is complete and the owner approves
