...
**Immediate next action — AURA-307 read-only discovery only.** With AURA-306 merged, the next task is **AURA-307 (Legal pages admin — draft→publish, versioning, audit; D-10/D-12)** — the seventh Phase 3 task and the **Phase 3 exit gate**. It is **not started**. **Do not start AURA-307 implementation directly. Do not create the AURA-307 branch until discovery is complete and the owner approves.** Read-only discovery only; a new session + explicit per-task discovery/planning approval is required before any work begins. AURA-307 touches the legal-content render/authoring boundary (D-12 merge blocker — no unsafe/raw HTML) and will require Opus review.
...
### Phase 3 — Admin Vertical Slice — In progress (6/7)
...
| **AURA-307** | Legal pages admin — draft→publish, versioning, audit (D-10/D-12) — **Phase 3 exit gate** | Not started — **next**; read-only discovery only; requires a new session + per-task discovery/planning approval |
...
- Do not start AURA-307 implementation directly — AURA-307 (Legal pages admin — draft→publish, versioning, audit; D-10/D-12; the seventh Phase 3 task and the Phase 3 exit gate) is **read-only discovery only**; it requires a new session + explicit per-task discovery/planning approval before implementation
- Do not create the AURA-307 branch until discovery is complete and the owner approves
