...
**Branch:** `develop` — source of truth at `74da365`. **Phase 1 is COMPLETE; Phase 2 (Public Website) is COMPLETE (7 of 7); Phase 3 (Admin Vertical Slice) is COMPLETE (7 of 7) — AURA-301 + AURA-302 + AURA-303 + AURA-304 + AURA-305 + AURA-306 + AURA-307 merged.** **AURA-307 (Legal pages admin — draft→publish, versioning, audit; D-10/D-12) — the seventh Phase 3 task and the Phase 3 exit gate — MERGED at `74da365`** (PR #51 squash-merged; `feat: add admin legal page management`; initial Opus 4.8 review **REQUEST_CHANGES** on a `data:`-prose false-positive in the write-time D-12 guard, fixed in `0b07090`; **re-review APPROVE, merge recommendation YES, no blocking issues**; required checks green before merge; feature branch `feature/aura-307-legal-admin` deleted local + remote). **With AURA-307 merged, Phase 3 is COMPLETE; the next task is AURA-401 (Inquiry + contact forms + `POST /api/leads`, validated + rate-limited) — the first Phase 4 (Lead and WhatsApp Conversion) task; not started; read-only discovery only.** **AURA-306 (Settings admin (allowlist + per-key Zod + audit)) MERGED at `86e8b36`** (PR #49 squash-merged; `feat: add admin settings management`; **targeted Opus 4.8 review APPROVE, merge recommendation YES, no blocking issues, no required fixes**; required checks green before merge; feature branch `feature/aura-306-settings-admin` deleted local + remote). **AURA-303 (Property CRUD admin + publish checklist) MERGED at `a6cb178`** (PR #43 squash-merged; `feat: add admin property CRUD`; **targeted Opus 4.8 review APPROVE, merge recommendation YES, no blocking issues, no required fixes**; required checks green before merge; feature branch `feature/aura-303-property-crud-admin` deleted local + remote). **AURA-302 (Admin dashboard shell) MERGED at `df4523c`** (PR #41 squash-merged; `feat: add admin dashboard shell`; **Opus review not required per the AURA-302 task block**; required checks green before merge; feature branch `feature/aura-302-admin-dashboard-shell` deleted local + remote). **AURA-207 (About page (`/en/about`) + Phase 2 public-page completion) MERGED at `65cc384`** (PR #37 squash-merged; `feat: add public about page`; **Opus review not required per the AURA-207 task block**; required checks green before merge; feature branch `feature/aura-207-about-page` deleted local + remote). **AURA-206 (SEO basics + AUTEX `noindex` (D-42) + enable Lighthouse advisory CI) MERGED at `a106fe8`** (PR #35 squash-merged; `feat: add SEO noindex and Lighthouse advisory`; **Opus review not required per the AURA-206 task block**; required checks green before merge; feature branch `feature/aura-206-seo-noindex-lighthouse` deleted local + remote). **AURA-205 (public legal page read — published-only legal DAL + `GET /api/legal/[slug]` + `/en/privacy` + `/en/terms` + safe Markdown render under D-12) MERGED at `3d6a7e0`** (PR #33 squash-merged; targeted Opus 4.8 review **APPROVE**, merge recommendation **YES**, no blocking issues; D-12 merge blocker satisfied; feature branch `feature/aura-205-legal-page-read` deleted local + remote). **AURA-201 (public `/[locale]` layout + header/footer/navigation + minimal next-intl v4 i18n shell + server-only public settings selector) MERGED at `f17b429`** (PR #25 squash-merged; targeted Opus 4.8 review **APPROVE**, merge recommendation **YES**, no blocking issues; feature branch `feature/aura-201-public-layout-i18n-shell` deleted local + remote). **AURA-107 (live DAL/security/integration tests in CI via Dockerized Supabase — Phase 1 exit gate) MERGED at `04d3522`** (PR #23 squash-merged; Opus 4.8 phase-exit review **APPROVE**, merge recommendation **YES**, no blocking issues; feature branch `feature/aura-107-dal-security-ci-harness` deleted). AURA-106 merged at `dd21edd`; AURA-105 at `fae3d62`; AURA-104 at `44a7fd4`; AURA-103 at `1a35958`; AURA-102 at `3657e4f`. **AURA-202 (public properties listing + `GET /api/properties` + featured) MERGED at `1d4c514`** (PR #27 squash-merged; merged 2026-06-22T12:54:55Z; targeted Opus 4.8 review **APPROVE**, merge recommendation **YES**, no blocking issues; feature branch deleted). **AURA-203 (public property detail + `GET /api/properties/[slug]` + stakeholder visibility + contact routing + off-plan) MERGED at `b2f6129`** (PR #29 squash-merged; targeted Opus 4.8 review **APPROVE**, merge recommendation **YES**, no blocking issues; feature branch `feature/aura-203-property-detail` deleted local + remote). **AURA-204 (public areas overview + `GET /api/areas` + `/[locale]/areas` overview page + public-safe area DTO + D-44 states) MERGED at `1fe2798`** (PR #31 squash-merged; targeted Opus 4.8 review **APPROVE**, merge recommendation **YES**, no blocking issues; feature branch `feature/aura-204-areas-overview` deleted local + remote). **AURA-301 (Admin login + session + role guard wiring) — the first Phase 3 (Admin Vertical Slice) task — MERGED at `97c9548`** (PR #39 squash-merged; `feat: add admin login and guard wiring`; targeted Opus 4.8 review **APPROVE**, merge recommendation **YES**, no blocking issues; feature branch `feature/aura-301-admin-login` deleted local + remote). **AURA-304 (Media upload — validation, UUID paths) — the fourth Phase 3 task — MERGED at `631bd29`** (PR #45 squash-merged; `feat: add admin media upload`; **targeted Opus 4.8 review APPROVE, merge recommendation YES, no blocking issues, no required fixes**; required checks green before merge; feature branch `feature/aura-304-media-upload` deleted local + remote). **AURA-305 (Areas admin (add/edit/deactivate)) — the fifth Phase 3 task — MERGED at `aee1fda`** (PR #47 squash-merged; `feat: add admin areas management`; **targeted Opus 4.8 review APPROVE, merge recommendation YES, no blocking issues, no required fixes**; required checks green before merge; feature branch `feature/aura-305-areas-admin` deleted local + remote). **Next task: AURA-401 (Inquiry + contact forms + `POST /api/leads`, validated + rate-limited) — the first Phase 4 (Lead and WhatsApp Conversion) task; not started; read-only discovery only, requires its own per-task discovery/planning approval.**

---

## AURA-307 — MERGED (`74da365`) — **PHASE 3 (7/7) → PHASE 3 COMPLETE**

**AURA-307: Legal pages admin — draft→publish, versioning, audit (D-10/D-12).** The seventh and final Phase 3 (Admin Vertical Slice) task and the **Phase 3 exit gate**: admin **create / edit / publish / archive** of the Privacy and Terms legal pages under a **row-per-version** lifecycle, with **write-time D-12 content safety** (Markdown only; no raw HTML) and audit logging. **No migration, no DB RPC, no Supabase config change, no `package.json`/`package-lock.json` change, no CI workflow change, no branch-protection change, no new renderer (existing `SafeMarkdown` reused), no `dangerouslySetInnerHTML` / `rehype-raw` / `marked` / DOMPurify, no arbitrary slugs, no hard delete, and no `legal_page_updated` audit action.**

Merged via PR #51 (squash) into `develop` at `74da365 feat: add admin legal page management`. Feature branch `feature/aura-307-legal-admin` deleted (local + remote). **Opus 4.8 review (PR #51):** initial **REQUEST_CHANGES** (a `data:`-prose false-positive in the write-time D-12 guard); after the blocker fix `0b07090 fix: narrow legal protocol guard to URL contexts`, **re-review APPROVE, merge recommendation YES, original blocker RESOLVED, no blocking issues, no required fixes**. Required checks passed before merge: `CodeQL`, `analyze (javascript-typescript)`, `quality`, `e2e`, `db-tests` (Lighthouse advisory non-blocking). Current source of truth is `develop` at `74da365`.

### Implementation summary

- **Guarded admin legal UI** — `src/app/admin/(protected)/legal/**` (list `page.tsx` + `loading.tsx` + `error.tsx`, `new/page.tsx`, `[id]/edit/page.tsx`). Inside the `(protected)` group, wrapped by the AURA-301 server-side fail-closed guard; there is **no unguarded `src/app/admin/legal/**`**. The content editor is a **Markdown textarea only** (`LegalPageForm`) — no rich-text / raw-HTML editor — whose optional preview reuses the public `SafeMarkdown`; published/archived versions render read-only. Row actions (`LegalPageRowActions`) publish/archive through the guarded API. All D-44 states handled.
- **Admin-only API routes** — `GET/POST /api/admin/legal`, `PATCH /api/admin/legal/[id]`, `POST /api/admin/legal/[id]/publish`, `POST /api/admin/legal/[id]/archive` (via the shared `withAdmin` helper in `src/app/api/admin/legal/_helpers.ts`). **Every route calls `requireAdmin()` directly** (not `requireSuperAdmin()`); both `super_admin` and `client_admin`; all input Zod-validated; safe `{ error, code }` envelopes (401 / 403 / 400 / 404 / 409 / generic 500).
- **Pure domain contract** — `src/domain/legal/admin.ts`: the `privacy` / `terms` slug allowlist, the **write-time D-12 content guard** (`containsUnsafeLegalHtml` / `assertSafeLegalMarkdown` — rejects `<script>` / `<iframe>` / `<style>` / raw tags / event handlers / HTML comments and dangerous protocol link targets, while accepting ordinary prose incl. `data:` / `personal data:` / `metadata:` and `5 < 10` after the `0b07090` fix), the create/update Zod schemas, the `nextPublishedVersion` bump rule, and the admin DTO projections. Pure TypeScript (no React/Supabase/DAL/I/O).
- **Admin legal DAL** — `src/dal/legal.dal.ts` extended with `listAdminLegalPages` / `getAdminLegalPageById` / `createLegalDraft` / `updateLegalDraft` / `publishLegalPage` / `archiveLegalPage` using a **request-scoped authenticated admin Supabase client + RLS — no service role**. The public `getPublishedLegalPage` selector is intact (anon client, published-only, explicit column allowlist, no `select('*')`). No `.delete()`; content is re-checked with `assertSafeLegalMarkdown` before every insert/update.
- **Publish sequence (row-per-version)** — load the selected draft → read all versions for the slug → **archive the currently published row for the same slug first** (keeps the partial unique index `legal_pages_slug_published_key` satisfied) → promote the selected draft to `published` with `version = max(existing version for slug) + 1` and `published_at = now`. Steps are separate writes; any failure **throws (fails loud)** — the owner-accepted non-atomic caveat, tested.
- **Audit** — `src/dal/audit-logs.dal.ts` extended with exactly `legal_page_created` / `legal_page_published` / `legal_page_archived` (entity type `legal_page`; metadata = slug / status transition / version only — never the legal title or body). `legal_page_updated` intentionally **not** added — draft PATCH is not audited. Minimal audit-action-union extension (no broad audit refactor); the append-only service-role writer is otherwise unchanged.

### Auth / RLS / service-role boundary

- Every `/api/admin/legal*` handler calls `requireAdmin()` through `withAdmin`; both `super_admin` and `client_admin` can create / edit / publish / archive legal pages; **no `requireSuperAdmin()`**. Public users cannot write legal pages; public legal reads stay published-only and draft/archived pages remain hidden publicly.
- Admin legal CRUD uses a **request-scoped authenticated admin Supabase client + RLS** (`legal_pages_admin_*` policies gated on `public.is_admin()`) — **no service role for admin legal CRUD**. The single service-role path remains the AURA-303 append-only audit writer. Existing `legal_pages` table + RLS + partial unique index were sufficient — **no migration, no Supabase config change.**

### D-12 safety

- Content is **Markdown only**; raw/unsafe HTML is **rejected at write time** (the domain guard on the Zod refine **and** re-asserted in the DAL) as well as at render. The existing `SafeMarkdown` (`react-markdown` + `rehype-sanitize`) remains the only renderer — no `dangerouslySetInnerHTML`, no `rehype-raw`, no `marked`, no DOMPurify/raw-innerHTML pipeline. The `0b07090` blocker fix narrowed the protocol guard to URL/link contexts so ordinary `data:` / `personal data:` / `metadata:` prose is accepted while dangerous protocol link targets (`[bad](javascript:alert(1))`, `<javascript:alert(1)>`) stay rejected. **D-12 remains a merge blocker.**

### Opus review summary

- Initial verdict **REQUEST_CHANGES** (blocker: the write-time guard false-positived on ordinary privacy-policy prose containing `data:`). After blocker fix `0b07090`: **APPROVE · Merge recommendation YES · Original blocker RESOLVED · Blocking issues None · Required fixes None**. A non-blocking follow-up was noted (reference-style link definitions `[foo]: javascript:…` are not flagged at write time but are neutralized by `rehype-sanitize` at render — no XSS path).

### Tests / gates summary

- **Tests added:** unit (`legal-admin-schema`), integration (`admin-legal-api`), security boundary (`admin-legal-boundary`, `legal-public-boundary` regression), live-DB DAL/RLS (`admin-legal.dal`, gated `SUPABASE_LOCAL_TESTS=1`), and e2e (`admin-legal` — unauthenticated `/admin/legal*` → `/admin/login` unconditional; the seeded-admin create→publish→archive happy path gated behind `ADMIN_E2E_EMAIL`/`ADMIN_E2E_PASSWORD` and skipped in CI). The blocker fix added `data:`-prose accept tests (unit + integration) and protocol-link reject tests.
- CI checks green before merge (`CodeQL`, `analyze (javascript-typescript)`, `db-tests`, `e2e`, `quality`; Lighthouse advisory non-blocking). The gated live suites ran in CI's `db-tests` job.

### Next safe action

**Phase 3 is COMPLETE (7 of 7).** The next task is **AURA-401 (Inquiry + contact forms + `POST /api/leads`, validated + rate-limited) — the first Phase 4 (Lead and WhatsApp Conversion) task — read-only discovery only.** Not started; requires a new session + explicit per-task discovery/planning approval before any work begins. **Do not start AURA-401 implementation** and **do not create the AURA-401 branch** until discovery is complete and the owner approves.

### Carry-forward / non-blocking (preserved for future tasks, not actioned at merge)

1. **Reference-style link definitions not flagged at write time** — `[foo]: javascript:alert(1)` (reference-link definition syntax) is not caught by the URL-context write-time guard, but `rehype-sanitize` strips the dangerous protocol from the rendered href, so there is no XSS path; a future hardening could add a reference-definition pattern for belt-and-braces write-time coverage.
2. **Publish/audit non-atomic ordering** — the publish sequence + audit write are separate writes (no migration/RPC); any failure is loud (throws → generic 500), never silently inconsistent. Matches the AURA-303 pattern; a future hardening may move the sequence into one transaction/RPC.
3. **Docs-sync completed in this PR** — `CURRENT_STATE.md`, `SESSION_HANDOFF.md`, `NEXT_STEPS.md`, `AURA_PROJECT_STATUS.md`, `docs/API_SPEC.md`, `docs/DATA_MODEL.md`, `docs/FEATURE_SPECS.md`, `docs/RBAC.md`, `docs/SECURITY_BASELINE.md`, `docs/TEST_STRATEGY.md`, and `docs/DECISION_LOG.md` updated to mark AURA-307 merged and Phase 3 complete. **Required checks unchanged** (`quality`, `e2e`, `analyze (javascript-typescript)`, `CodeQL`, `db-tests`); `docs/QUALITY_GATES.md`, `docs/BRANCH_PROTECTION.md`, and `docs/TASKS_Project.md` untouched.

---

## AURA-306 — MERGED (`86e8b36`) — **PHASE 3 (6/7)**

**AURA-306: Settings admin (allowlist + per-key Zod + audit).** The sixth Phase 3 (Admin Vertical Slice) task: admin **read + update** of the operational footer settings via a server-side key-value allowlist, per-key Zod validation, a partial-batch PATCH, and audit logging — updates are **immediate** (no draft/publish flow). **No migration, no Supabase config change, no `package.json`/`package-lock.json` change, no CI workflow change, no branch-protection change, no public settings API, no settings delete path, no service-role for admin settings GET/PATCH, and no logo/SEO/legal/trust/design-token scope (all deferred).**

Merged via PR #49 (squash) into `develop` at `86e8b36 feat: add admin settings management`. Feature branch `feature/aura-306-settings-admin` deleted (local + remote). **Targeted Opus 4.8 review (PR #49): APPROVE, merge recommendation YES, no blocking issues, no required fixes before merge** (two non-blocking follow-ups preserved below). Required checks passed before merge: `CodeQL`, `analyze (javascript-typescript)`, `quality`, `e2e`, `db-tests` (Lighthouse advisory non-blocking). Current source of truth is `develop` at `86e8b36`.

### Implementation summary

- **Guarded admin settings page** — `src/app/admin/(protected)/settings/page.tsx` (`/admin/settings`), plus `loading.tsx` / `error.tsx`. Inside the `(protected)` group, wrapped by the AURA-301 server-side fail-closed guard. Reads via the admin DAL (admin's own session + RLS; no service role) and renders the `SettingsForm` client island.
- **Admin-only API routes** — `GET /api/admin/settings`, `PATCH /api/admin/settings` (via the shared `withAdmin` helper in `src/app/api/admin/settings/_helpers.ts`). **Every route calls `requireAdmin()` directly** (not `requireSuperAdmin()`); both `super_admin` and `client_admin` may edit all allowed keys; all input Zod-validated; safe `{ error, code }` envelopes (401 / 403 / generic 500).
- **Editable allowlist (exactly the seven public footer keys)** — `agency_name`, `agency_phone`, `agency_email`, `agency_whatsapp`, `agency_address`, `footer_tagline`, `social_links` (canonical names; the same set the public selector projects — a compile-time backstop asserts editable == public allowlist).
- **Per-key Zod + partial batch** — pure domain contract `src/domain/settings/admin.ts` (`EDITABLE_SETTING_KEYS`, `settingsPatchSchema`, `toSettingRows`): a **strict** schema that **rejects unknown / deferred keys**, per-key value grammar (non-empty trimmed strings, valid email, known social-platform URLs; social `{}` clears links), and an **empty-patch rejection (400)**. `PATCH` writes via `upsert(onConflict: 'key')`.
...
- **Audit** — `src/dal/audit-logs.dal.ts` extended with exactly `settings_updated` (entity type `settings`; metadata `{ changed_keys }` — changed key **names only**, never values) — a minimal audit-action-union extension (no broad audit refactor); the post-mutation/non-atomic ordering caveat matches the AURA-303 pattern.
...
- Every `/api/admin/settings` handler calls `requireAdmin()` through `withAdmin`; both `super_admin` and `client_admin` can read and update all allowed AURA-306 settings; **no `requireSuperAdmin()`**. Public users cannot read/write the admin settings endpoints; the public surface receives only safe, server-projected settings via the existing AURA-201 selector.
- Admin settings GET/PATCH use a **request-scoped authenticated admin Supabase client + RLS** (`settings_admin_select/insert/update` gated on `public.is_admin()`) — **no service role for admin settings read/write**. The existing public safe selector (`getPublicSettings`) legitimately keeps the service role, and the only other service-role path remains the AURA-303 append-only audit writer. Existing `settings` table + RLS were sufficient: authenticated admin can `select/insert/update`; anon has no direct settings access; no DELETE policy — **no migration, no Supabase config change.**

### Public behaviour

- The public footer reflects allowed settings updates on the **next request** (the `[locale]` layout is `force-dynamic` and reads settings per request). **No cache/revalidation system added, no public settings API added, no public SEO metadata wiring added.** Existing public settings behaviour is otherwise unchanged.

### Opus review summary

- Verdict **APPROVE** · Merge recommendation **YES** · Blocking issues **None** · Required fixes before merge **None**.
- The settings governance + audit boundary was cleared: allowlist == seven footer keys, unknown/deferred keys + empty patch rejected, per-key Zod, request-scoped admin client + RLS (no service-role for admin GET/PATCH), `settings_updated` audit with changed key names only, no settings delete, no public settings API, and no template/design-architecture mutation (D-21).

### Tests / gates summary

- **Tests added:** unit (`settings-admin-schema`), integration (`admin-settings-api`), security boundary (`admin-settings-boundary`), live-DB DAL/RLS (`admin-settings.dal`, gated `SUPABASE_LOCAL_TESTS=1` — admin SELECT/INSERT/UPDATE + read-back, anon denied on all three, hard-delete denied for admin), and e2e (`admin-settings` — unauthenticated `/admin/settings` → `/admin/login` unconditional; seeded-admin happy path edit→save→public-footer gated behind `ADMIN_E2E_EMAIL`/`ADMIN_E2E_PASSWORD` and skipped in CI).
- CI checks green before merge (`CodeQL`, `analyze (javascript-typescript)`, `db-tests`, `e2e`, `quality`; Lighthouse advisory non-blocking). The gated live suites ran in CI's `db-tests` job.

### Next safe action

**AURA-307 (Legal pages admin — draft→publish, versioning, audit; D-10/D-12) — the seventh Phase 3 task and the Phase 3 exit gate — read-only discovery only.** Not started; requires a new session + explicit per-task discovery/planning approval before any work begins. **Do not start AURA-307 implementation** and **do not create the AURA-307 branch** until discovery is complete and the owner approves. AURA-307 touches the legal-content authoring/render boundary (D-12 merge blocker — no unsafe/raw HTML) and will require Opus review.
...
3. **Docs-sync completed in this PR** — `CURRENT_STATE.md`, `SESSION_HANDOFF.md`, `NEXT_STEPS.md`, `docs/API_SPEC.md`, `docs/DATA_MODEL.md`, `docs/FEATURE_SPECS.md`, `docs/RBAC.md`, `docs/SECURITY_BASELINE.md`, `docs/TEST_STRATEGY.md`, and `docs/DECISION_LOG.md` updated to mark AURA-306 merged. **Required checks unchanged** (`quality`, `e2e`, `analyze (javascript-typescript)`, `CodeQL`, `db-tests`); `docs/QUALITY_GATES.md`, `docs/BRANCH_PROTECTION.md`, and `docs/TASKS_Project.md` untouched.

---

## AURA-305 — MERGED (`aee1fda`) — **PHASE 3 (5/7)**

**AURA-305: Areas admin (add/edit/deactivate).** The fifth Phase 3 (Admin Vertical Slice) task: admin **add / edit / deactivate / reactivate** of areas (communities) with one representative area/community image, admin-only per-area property counts, and audit logging — **no hard delete**. **No migration, no Supabase config change, no `package.json`/`package-lock.json` change, no CI workflow change, no branch-protection change, no area media table, no gallery, no multi-upload/drag-drop, no image processing/resizing/transcoding, no public property counts, no public areas redesign, no public area-detail pages, no AURA-306+ scope.**

Merged via PR #47 (squash) into `develop` at `aee1fda feat: add admin areas management`. Feature branch `feature/aura-305-areas-admin` deleted (local + remote). **Targeted Opus 4.8 review (PR #47): APPROVE, merge recommendation YES, no blocking issues, no required fixes before merge** (two non-blocking follow-ups preserved below). Required checks passed before merge: `CodeQL`, `analyze (javascript-typescript)`, `quality`, `e2e`, `db-tests` (Lighthouse advisory non-blocking). Current source of truth is `develop` at `aee1fda`.

### Implementation summary

- **Admin areas pages** — `src/app/admin/(protected)/areas/page.tsx` (`/admin/areas`), `…/areas/new/page.tsx` (`/admin/areas/new`), `…/areas/[id]/edit/page.tsx` (`/admin/areas/[id]/edit`), plus `error.tsx` / `loading.tsx`. All inside the `(protected)` group, wrapped by the AURA-301 server-side fail-closed guard.
- **Admin-only API routes** — `GET /api/admin/areas`, `POST /api/admin/areas`, `PATCH /api/admin/areas/[id]` (via the shared `withAdmin` helper in `src/app/api/admin/areas/_helpers.ts`). **Every route calls `requireAdmin()` directly** (not `requireSuperAdmin()`); both `super_admin` and `client_admin`; all input Zod-validated; generic `{ error, code }` errors.
- **Area lifecycle** — create / edit / **deactivate** (`is_active = false`) / **reactivate** (`is_active = true`); **no hard delete** (no `DELETE` endpoint, no `.delete()`). The **slug is editable only at create — `PATCH` cannot update the slug.** The admin UI uses an **inline two-step deactivate confirmation** (`AreaRowActions`); `sort_order` and active/inactive status are in the form (`AreaForm`, `AreaStatusBadge`).
- **Admin-only property counts** — admin list rows carry `totalProperties` / `publishedProperties` per area (computed from `properties.area_id` + `publish_status`; not stored; never public).
- **Representative area image** — one single image upload reusing the **existing `property-media` bucket** under the **server-built, UUID-only** path `areas/{area_id}/{image_id}.{ext}` (extension from MIME; original filename never trusted; MIME allowlist `image/jpeg` / `image/png` / `image/webp`; 10MB max; `upsert: false`); the **public URL is stored in `areas.image_url`** — no area media table, no gallery, no multi-upload.
- **Admin areas DAL + domain + storage service** — `src/dal/admin-areas.dal.ts` (caller's own admin session + RLS, **no service role**), pure domain `src/domain/areas/{admin,admin-view}.ts` (Zod contracts + the admin view/projector incl. counts), the server-only storage service `src/services/storage/area-image.ts`, and the `AreaForm` / `AreaRowActions` / `AreaStatusBadge` admin components.
- **Audit** — `src/dal/audit-logs.dal.ts` extended with `area_created` / `area_updated` (deactivate/reactivate logged as `area_updated` with metadata) — a minimal audit-action-union extension (no broad audit refactor); the post-mutation/non-atomic ordering caveat matches the AURA-303 pattern.

### Auth / RLS / storage boundary

- Every `/api/admin/areas*` route calls `requireAdmin()` through `withAdmin`; both `super_admin` and `client_admin` can manage areas; **no `requireSuperAdmin()`**. Public users cannot create / edit / deactivate / reactivate areas; public `/api/areas` stays **active-only** and inactive areas stay hidden; property counts are admin-only.
- Area CRUD and the area image upload use a **request-scoped authenticated admin Supabase client + RLS — no service role for area CRUD/upload**; service-role stays isolated to the audit writer. The image reuses the AURA-105 `property-media` bucket (authenticated admin writes allowed; anon write/list/mutation denied) at a server-built UUID-only path; original filename is never trusted.

### Opus review summary

- Verdict **APPROVE** · Merge recommendation **YES** · Blocking issues **None** · Required fixes before merge **None**.
- The area write boundary was cleared: admin-only routes (`requireAdmin()` directly; no public create/edit/deactivate/reactivate), request-scoped authenticated admin session + existing RLS for area CRUD and image upload (no service role for area work), server-built UUID storage paths under the existing `property-media` bucket (no enumeration), MIME + size validation, public active-only visibility, and admin-only property counts.

### Tests / gates summary

- **Tests added/updated:** unit (`area-admin-schema`, `area-admin-view`), integration (`admin-areas-api`), security boundary (`admin-areas-boundary`; `areas-public-boundary` updated for AURA-305), live-DB DAL/RLS/storage (`admin-areas.dal`, gated `SUPABASE_LOCAL_TESTS=1`), and e2e (`admin-areas`, gated authenticated happy path behind `ADMIN_E2E_EMAIL`/`ADMIN_E2E_PASSWORD`, skipped in CI).
- CI checks green before merge (`CodeQL`, `analyze (javascript-typescript)`, `db-tests`, `e2e`, `quality`; Lighthouse advisory non-blocking). The gated live suites ran in CI's `db-tests` job.

### Next safe action

**AURA-306 (Settings admin (allowlist + per-key Zod + audit)) — the sixth Phase 3 task — read-only discovery only.** Not started; requires a new session + explicit per-task discovery/planning approval before any work begins. **Do not start AURA-306 implementation** and **do not create the AURA-306 branch** until discovery is complete and the owner approves. No implementation until owner approval.
...
2. **Best-effort old-image cleanup** — only the public URL is stored in `areas.image_url`, so cleaning up a replaced/old area image is best-effort; future signed URLs / object revocation / storage GC may address both this and the known-URL fetchability above.
3. **Docs-sync completed in this PR** — `CURRENT_STATE.md`, `SESSION_HANDOFF.md`, `NEXT_STEPS.md`, `docs/API_SPEC.md`, `docs/DATA_MODEL.md`, `docs/FEATURE_SPECS.md`, `docs/RBAC.md`, `docs/SECURITY_BASELINE.md`, `docs/TEST_STRATEGY.md`, and `docs/DECISION_LOG.md` updated to mark AURA-305 merged. **Required checks unchanged** (`quality`, `e2e`, `analyze (javascript-typescript)`, `CodeQL`, `db-tests`); `docs/QUALITY_GATES.md` and `docs/BRANCH_PROTECTION.md` untouched.

---

## AURA-304 — MERGED (`631bd29`) — **PHASE 3 (4/7)**

**AURA-304: Media upload — validation, UUID paths.** The fourth Phase 3 (Admin Vertical Slice) task: admin **upload / update / delete** of property media (images + floorplan images) with type/size validation, server-built UUID storage paths, and cover-image/alt-text controls — the first real consumer of the AURA-105 storage policies + media path contract. **No migration, no Supabase config change, no `package.json`/`package-lock.json` change, no CI workflow change, no branch-protection change, no signed URLs, no CDN revocation, no video/360/virtual-tour, no image processing/resizing/transcoding, no manual reordering, no multi-file upload/drag-drop, no media audit actions, no AURA-305+ scope.**

Merged via PR #45 (squash) into `develop` at `631bd29 feat: add admin media upload`. Feature branch `feature/aura-304-media-upload` deleted (local + remote). **Targeted Opus 4.8 review (PR #45): APPROVE, merge recommendation YES, no blocking issues, no required fixes before merge** (three non-blocking follow-ups preserved below). Required checks passed before merge: `CodeQL`, `analyze (javascript-typescript)`, `quality`, `e2e`, `db-tests` (Lighthouse advisory non-blocking). Current source of truth is `develop` at `631bd29`.
...
- **Admin media manager UI** — `src/components/admin/PropertyMediaManager.tsx`, wired into `src/app/admin/(protected)/properties/[id]/edit/page.tsx`: upload, alt-text editing, cover selection.
...
- **Storage-write mechanism** — a **request-scoped authenticated admin Supabase client + existing RLS — no service role for media** (the AURA-303 service-role boundary, the audit writer, is unchanged).
...
- Verdict **APPROVE** · Merge recommendation **YES** · Blocking issues **None** · Required fixes before merge **None**.
- The media write boundary was cleared: admin-only routes (`requireAdmin()` directly; no public upload/update/delete), request-scoped authenticated admin session + existing RLS for storage writes (no service role for media), server-built UUID paths (no enumeration), MIME + size validation, and published-parent-only public media read (draft/archived media not public). Signed URLs / CDN revocation remain deferred.
...
- **Gated tests:** the live DAL/RLS media suites run under `SUPABASE_LOCAL_TESTS=1` (they ran in CI's `db-tests` job); the e2e admin media flow is gated behind seeded-admin `ADMIN_E2E_EMAIL` / `ADMIN_E2E_PASSWORD` and skipped in CI.
- PR checks green (`CodeQL`, `analyze (javascript-typescript)`, `quality`, `e2e`, `db-tests`).

### Next safe action

**AURA-305 (Areas admin (add/edit/deactivate)) — the fifth Phase 3 task — read-only discovery only.** Not started; requires a new session + explicit per-task discovery/planning approval before any work begins. **Do not start AURA-305 implementation** and **do not create the AURA-305 branch** until discovery is complete and the owner approves. No implementation until owner approval.
...
3. **Public-read bucket CDN revocation gap remains deferred** — a retained object URL stays fetchable after a property is unpublished/archived; track toward the deferred signed-URL / CDN work.
4. **Docs-sync completed in this PR** — `CURRENT_STATE.md`, `SESSION_HANDOFF.md`, `NEXT_STEPS.md`, `docs/API_SPEC.md`, `docs/DATA_MODEL.md`, `docs/FEATURE_SPECS.md`, `docs/SECURITY_BASELINE.md`, `docs/RBAC.md`, `docs/TEST_STRATEGY.md`, and `docs/DECISION_LOG.md` updated to mark AURA-304 merged. **Required checks unchanged** (`quality`, `e2e`, `analyze (javascript-typescript)`, `CodeQL`, `db-tests`); `docs/BRANCH_PROTECTION.md` untouched.

---

## AURA-303 — MERGED (`a6cb178`) — **PHASE 3 (3/7)**

**AURA-303: Property CRUD admin + publish checklist.** The third Phase 3 (Admin Vertical Slice) task: admin property **create / edit / publish / archive / duplicate** with a publish-readiness checklist and append-only audit logging. The first task to introduce **admin write routes**, the **publish/visibility lifecycle**, **audit logging**, and the **service-role audit-log write path** — the four reasons it required Opus review. **No media upload (AURA-304), no stakeholder admin, no areas/settings/legal admin, no lead/contact/WhatsApp flows, no dashboard metrics, no hard delete, no unpublish flow, no migration, no Supabase config change, no package change, no CI workflow change, no branch-protection change.**

Merged via PR #43 (squash) into `develop` at `a6cb178 feat: add admin property CRUD`. Feature branch `feature/aura-303-property-crud-admin` deleted (local + remote). **Targeted Opus 4.8 review (PR #43): APPROVE, merge recommendation YES, no blocking issues, no required fixes before merge** (five non-blocking follow-ups preserved below). Required checks passed before merge: `CodeQL`, `analyze (javascript-typescript)`, `quality`, `e2e`, `db-tests`. Current source of truth is `develop` at `a6cb178`.

### Implementation summary

- **Guarded admin property pages** — `src/app/admin/(protected)/properties/{page,new/page,[id]/edit/page}.tsx` (plus `error.tsx` / `loading.tsx`): all under the `(protected)` group, wrapped by the AURA-301 server-side fail-closed guard. **No unguarded `src/app/admin/properties/**` route exists.**
- **Admin API routes (each calls `requireAdmin()` directly)** — `GET/POST /api/admin/properties`, `PATCH /api/admin/properties/[id]`, `POST /api/admin/properties/[id]/duplicate`, `PATCH /api/admin/properties/[id]/archive`, via the shared `withAdmin` helper (`src/app/api/admin/properties/_helpers.ts`). The `(protected)` layout guards pages, not Route Handlers, so each handler enforces the guard itself. Both `super_admin` and `client_admin` may manage properties (no property action is super-admin-only). All input Zod-validated; generic `{ error, code }` errors with no DB-detail leak.
- **Admin write DAL** — `src/dal/admin-properties.dal.ts` (`import 'server-only'`): admin reads (all statuses) + writes under the **caller's own admin session + existing RLS — never the service role**. Explicit column allowlists; no `.delete()`; no DELETE endpoint. The public `src/dal/properties.dal.ts` stays untouched / public-read-only (split mirrors AURA-203's `property-detail.dal.ts`).
- **Server-only audit DAL** — `src/dal/audit-logs.dal.ts` (`import 'server-only'`): the only application write path for `audit_logs`, using the service-role client (RLS grants `authenticated` no insert; super_admin SELECT only). Append-only; non-PII curated snapshots (D-38); throws on failure so a missing audit is **loud, never silently swallowed**.
- **Pure domain logic** — `src/domain/properties/{admin,admin-view,publish}.ts`: create/update/duplicate Zod contracts + taxonomy enums (D-36), slug derivation/immutability + reference-number formatting (A-06 / A-05 / D-47), lifecycle predicates (D-32), the `buildUpdatePayload` immutability backstop, the admin DTOs, and `evaluatePublishChecklist`.
- **Publish checklist** — blocks draft → published unless: English title + description; valid taxonomy; location label; price when `price_visibility = visible`; rental period for a visibly-priced rental; bedrooms for residential types (nullable for office/plot/retail/warehouse, D-09); off-plan fields only when `market_type = off_plan`; ≥1 cover image with non-empty alt text. **Media upload is NOT implemented — the checklist only reads existing `property_media` rows (upload is AURA-304).**
- **Lifecycle** — create = draft; edit draft; publish draft only if checklist passes; edit published except slug immutability; archive draft/published (`publish_status = archived`, the MVP way to remove from public view); archived rejects edits; **no `published → draft` unpublish; no hard delete.**
- **Audit actions** — `property_created`, `property_updated`, `property_published`, `property_archived`, `property_duplicated`.

### Opus review summary

- Verdict **APPROVE** · Merge recommendation **YES** · Blocking issues **None** · Required fixes before merge **None**.
- All six Opus-required risk areas cleared: admin write routes (in-handler `requireAdmin`), publish/visibility lifecycle, audit logging (loud-fail, non-PII), service-role boundary (only the server-only audit DAL; never in UI), public visibility boundary (anon = published-only; draft/archived hidden; archive removes from public view), and the RLS boundary (no properties DELETE policy; `audit_logs` insert default-deny for authenticated).

### Tests / gates summary

- **Tests added:** unit (`property-admin-domain`, `property-admin-schema`, `property-publish-checklist`), security boundary (`admin-properties-boundary`, `property-detail-public-boundary` regression), integration (`admin-properties-api`, DAL/auth/audit mocked), live-DB DAL (`admin-properties.dal`, gated `SUPABASE_LOCAL_TESTS=1`), e2e guard (`admin-properties`, unauthenticated → `/admin/login`; seeded-admin flow gated/skipped in CI).
- PR checks green (`CodeQL`, `analyze (javascript-typescript)`, `quality`, `e2e`, `db-tests`). The live DAL/security/integration boundary proofs (anon cannot read draft/archived; hard delete denied; admin cannot insert audit directly) ran in CI's `db-tests` job against the Dockerized Supabase stack; locally those suites self-skip without `SUPABASE_LOCAL_TESTS=1` (acceptable — CI `db-tests` is the enforced gate).

### Next safe action

**AURA-304 (Media upload — validation, UUID paths) — the fourth Phase 3 task — read-only discovery only.** Not started; requires a new session + explicit per-task discovery/planning approval before any work begins. **Do not start AURA-304 implementation** and **do not create the AURA-304 branch** until discovery is complete and the owner approves. AURA-304 is the first real importer of the AURA-105 media/storage modules (`src/domain/properties/media.ts`, `src/services/storage/policy.ts`) — remove their Knip `entry` lines then.

### Carry-forward / non-blocking (from the targeted Opus review; preserved for future tasks, not actioned at merge)

1. **Document `property_duplicated` as a controlled audit action** — it is emitted + tested but was not in the documented minimum-action list (`DATA_MODEL.md` / `SECURITY_BASELINE.md`); added in this docs-sync.
2. **Audit-write ordering caveat** — each route commits the property mutation **first**, then writes the audit row; an audit-insert failure is **loud** (generic 500, never silently swallowed) but does not roll back the committed change. A future hardening may move mutation + audit into a single transaction/RPC.
3. **Ratify no `published → draft` unpublish in MVP** — the one-way lifecycle is an implementation inference from D-32 (archive is the MVP way to remove a property from public view), not yet a named decision; recorded here for ratification.
4. **Add direct 401/403 integration assertions** for `PATCH /api/admin/properties/[id]` and `POST /api/admin/properties/[id]/duplicate` — their auth boundary is currently proven structurally (all routes go through `withAdmin` → `requireAdmin`) rather than with a per-verb behavioural assertion.
5. **Stale code-comment cleanup (future, code-only)** — a header comment in `src/domain/properties/admin.ts` references `src/dal/properties.dal.ts` where the actual write DAL is `src/dal/admin-properties.dal.ts`. Cosmetic; deferred to a future code cleanup (not touched in this docs-sync).
